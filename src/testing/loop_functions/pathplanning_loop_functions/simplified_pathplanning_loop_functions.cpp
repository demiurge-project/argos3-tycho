...
CSimplifiedPathPlanningLoopFunctions::CSimplifiedPathPlanningLoopFunctions() :
    m_vecDestinations(),
...
void CSimplifiedPathPlanningLoopFunctions::Init(TConfigurationNode& t_node) {
   try {
      ...
      if (m_strEntityType == "epuck"){
    	  m_fRobotBodyRadius = 0.035f;
  	  }
  	  else if (m_strEntityType == "foot-bot") {
      ...
      std::string strRobotType = (m_strEntityType == "epuck") ? "e-puck" : m_strEntityType;
      ...
      if (unNumberRobotEntityDefined != m_unTotalNumberRobot){
  		  LOGERR << "Incorrect number of robots defined, " << unNumberRobotEntityDefined << " robots detected, " << m_unTotalNumberRobot << " needed of type " << strRobotType << std::endl;
...
void CSimplifiedPathPlanningLoopFunctions::ParseFixedDestinations(TConfigurationNode&  tParametersNode) {
  /* Example of parameters for fixed destinations of 3 robots.
  <pathplanning randomDistribution="false" sharedDestination="true/false" robots="3" robotType="epuck/foot-bot">
    <destinations>
...
void CSimplifiedPathPlanningLoopFunctions::ParseRandomDistributionParameters(TConfigurationNode&  tParametersNode) {
...
void CSimplifiedPathPlanningLoopFunctions::PreStep() {
...
bool CSimplifiedPathPlanningLoopFunctions::IsExperimentFinished() {
...
void CSimplifiedPathPlanningLoopFunctions::PostExperiment() {
...
void CSimplifiedPathPlanningLoopFunctions::GenerateRandomDestinations(){
...
void CSimplifiedPathPlanningLoopFunctions::Reset() {
...
void CSimplifiedPathPlanningLoopFunctions::Destroy() {
	//delete m_pcFloor;
...
CColor CSimplifiedPathPlanningLoopFunctions::GetFloorColor(const CVector2& c_position_on_plane) {
  return CColor::WHITE;
}
...
void CSimplifiedPathPlanningLoopFunctions::PostStep() {
...
std::vector<CVirtualSensorDataPosition2D> CSimplifiedPathPlanningLoopFunctions::GetPathForController(const CVoidController& c_controller){
...
REGISTER_LOOP_FUNCTIONS(CSimplifiedPathPlanningLoopFunctions, "simplified_pathplanning_loop_functions")
